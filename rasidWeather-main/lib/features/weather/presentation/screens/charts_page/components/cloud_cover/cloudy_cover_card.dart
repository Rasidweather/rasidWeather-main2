...
import 'cloudy_cover_sheet.dart';

class CloudCoverWidget extends StatelessWidget {
  const CloudCoverWidget({super.key, required this.day});
  ...
  Widget build(BuildContext context) {
    ...
    return WeatherContainer(
      ...
      content: GestureDetector(
        onTap: () {
          navigateDialog(context: context, child: CloudyDialog(hours: hours));
        },
        child: Column(
          children: <Widget>[
  ...
  Widget _buildHeader(Hour hour) {
    return ListTile(
      ...
      title: Row(
        children: <Widget>[
          // Cloud icon with background
          _buildCloudIcon(),

          const Spacer(),

          // Current cloud cover value
  ...
  Widget _buildCurrentValue(Hour hour) {
    return ForecastDisplay(
      value: hour.cloudCover!.toString().removeZeroLeftDecimalPercentage(),
      unit: '%',
      unitSizeFactor: 1.6,
      style: TextStyle(
        fontSize: 22.sp,
  ...
  Widget _buildTitleSection() {
    return ListTile(
      ...
      subtitle: Text(
        'weather.charts.clouds.subtitle'.tr(),
        style: TextStyle(
          fontSize: 9.sp,
          fontWeight: FontWeight.w400,
