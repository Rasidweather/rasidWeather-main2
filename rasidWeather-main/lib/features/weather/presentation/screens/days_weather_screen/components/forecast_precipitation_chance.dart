...
/// A widget that displays precipitation chance information.
///
/// This widget shows:
...
class ForecastPrecipitationChance extends StatelessWidget {
  /// Creates a forecast precipitation chance widget.
  ///
  /// [precipitationChance] is required and represents the percentage chance of precipitation.
  /// [currentView] determines if this is showing current or forecast data.
  const ForecastPrecipitationChance({
    super.key,
  ...
  static const Color _kIconBackgroundColor = Color(0xff9266DA);
  ...
  /// Builds the precipitation chance percentage display.
  Widget _buildPrecipitationValue() {
    return ForecastMetaInfo(
      currentView: currentView,
      label: 'precipitationChance',
      value: precipitationChance.toString().removeZeroLeftDecimal(),
      unit: '%',
    );
  }
  ...
  Widget _buildDescription() {
    return ListTile(
      ...
      contentPadding: EdgeInsets.zero,
      title: Text(
        'weather.rain.precipitationChance'.tr(),
        style: TextStyle(
      ...
      isThreeLine: true,
      subtitle: Text(
        'weather.rain.precipitationChanceRate'.tr(),
        maxLines: 2,
