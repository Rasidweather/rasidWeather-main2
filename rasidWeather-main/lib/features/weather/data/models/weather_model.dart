...
class Appearance extends Equatable {
  ...
  factory Appearance.fromJson(Map<String, dynamic> json) {
    return Appearance(
      ...
      stops: json['stops'] == null ? const <double>[] : List<double>.from((json['stops'] as List<dynamic>).map((x) => x.toDouble())),
    );
  }
  
  /// Creates a copy of this Appearance with the given fields replaced with new values
  Appearance copyWith({
...
extension ConditionLocaleX on Condition {
  String localizedName({required bool isArabic}) {
    final String rawName = (conditionName ?? '').trim();
    final String rawCode = (conditionCode ?? '').trim();

    if (isArabic) {
      return rawName.isNotEmpty ? rawName : '—';
    }

    // EN: أولاً من الكود
    final String? byCode = _englishFromCode(rawCode);
    if (byCode != null) return byCode;

    // EN: إذا الكود مش معروف، جرّب ترجمة من النص العربي نفسه
    final String? byArabicName = _englishFromArabicName(rawName);
    if (byArabicName != null) return byArabicName;

    return 'Unknown';
  }

  String? _englishFromCode(String code) {
    if (code.isEmpty) return null;

    // ✅ عدّل/زِد حسب أكوادك الفعلية
  ...
  String? _englishFromArabicName(String name) {
    final String n = name.trim();
    if (n.isEmpty) return null;

    // ✅ ترجمات مباشرة من العربي اللي بيرجع عندك
    const Map<String, String> map = <String, String>{
